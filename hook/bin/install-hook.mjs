...
// Thân script chỉ chạy khi file này được gọi trực tiếp (node install-hook.mjs
// ...), không phải khi một bài test import nó để lấy lenhHook — thân này kết
// thúc bằng process.exit(), import nó mà không có guard là test tự sát.
...
if (chayTrucTiep) {
  ...
  const file = path.join(ccrcHome(), '.claude', 'settings.json');
